...
use gemini::{
	GeminiClient, GeminiMessage, GeminiPart, GeminiRequest, GeminiRole,
	GeminiSafetySetting, GeminiSafetyThreshold, GeminiSystemPart,
};
use mime::Mime;
use regex::Regex;
use reqwest::Url;
...
impl Claide {
	async fn process_message(
		...
		let previous_messages = {
			let Some(cached_messages) =
				context.cache.channel_messages(message.channel_id)
			else {
			...
			for message in messages {
				...
				attachments.extend(
					REGEX_URL
						...
						.filter(|url| {
							self.config.whitelisted_domains.url_matches(url)
						})
						.map(Attachment::Url),
				);
				attachments.extend(
		...
		let system = ["You are a discord user named Blinky"].join(" ");
		...
		let settings = [
			GeminiSafetySetting::Harassment,
			GeminiSafetySetting::HateSpeech,
			GeminiSafetySetting::SexuallyExplicit,
			GeminiSafetySetting::DangerousContent,
			GeminiSafetySetting::CivicIntegrity,
		];

		let settings =
			settings.map(|setting| (setting)(GeminiSafetyThreshold::BlockNone));

		request.safety_settings.extend(settings);

		for (role, text, attachments) in previous_messages {
			let attachment =
				attachments.into_iter().map(|attachment| async move {
					anyhow::Ok(
						match self.seen.entry(attachment.url().to_string()) {
							dashmap::Entry::Occupied(occupied) => {
								occupied.get().clone()
							}
							dashmap::Entry::Vacant(vacant) => {
								let pair =
									attachment.upload_into_gemini(self).await?;
		...
		let content = content.strip_prefix("Blinky:").unwrap_or(content).trim();
		...
		if content.chars().count() > 1950 {
			builder = builder
				.add_file(CreateAttachment::bytes(content, "message.txt"));
		} else {
