...
impl DomainMatcher {
	...
	{
		Ok(Self {
			backend: AhoCorasick::new(
				patterns.into_iter().map(|pattern| format!(".{pattern}")),
			)?,
		})
	}
...
mod tests {
	...
	fn domain_matcher() {
		let whitelist =
			DomainMatcher::new(vec!["example.com", "foo.bar"]).unwrap();
	...
	fn domain_matcher_url() {
		let whitelist =
			DomainMatcher::new(vec!["discord.gg", "google.com"]).unwrap();

		assert!(whitelist.url_matches(
			&"https://google.com/test".parse().expect("url parsing")
		));
		assert!(
			!whitelist.url_matches(&"foo:bar".parse().expect("url parsing"))
		);
	}
}
