...
const X_GOOG_UPLOAD_COMMAND: HeaderName =
...
pub enum GeminiSafetySetting {
	...
	HateSpeech(GeminiSafetyThreshold),
	SexuallyExplicit(GeminiSafetyThreshold),
	DangerousContent(GeminiSafetyThreshold),
	CivicIntegrity(GeminiSafetyThreshold),
}
...
impl GeminiClient {
	...
	pub async fn create_file(
		...
		let url = response
			...
			.ok_or_else(|| {
				anyhow::anyhow!("missing expected x-goog-upload-url")
			})?;
	...
	pub async fn upload_file(
		...
		while response.file.state == "PROCESSING" {
			...
			response.file = self
				...
				.await
				.inspect_err(|error| {
					tracing::error!("processing file: {error}")
				})?
				.json()
				.await
				.inspect_err(|error| {
	...
	pub async fn generate(
		...
		let url =
			self.with_base("v1beta/models/gemini-1.5-flash:generateContent");
		let query = [("key", &self.api_key)];
		...
		let result = serde_json::from_str::<GeminiResponse>(
